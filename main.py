...
def get_row_col_from_mouse(pos: Tuple[int, int], square_size: int) -> Tuple[int, int]:
    """Връща реда и колоната от позицията на мишката в координатите на дъската."""
    x, y = pos
    return y // square_size, x // square_size

def menu() -> Optional[str]:
    """Показва началното меню, позволява избор на режим на игра."""
    run = True
    selected_mode = None
...
def difficulty_menu(mode: str) -> Dict[str, int] | None:
    """Меню за избор на трудност в зависимост от режима на игра."""
    font = pygame.font.SysFont("comicsans", 40)
...
def color_menu(mode: str) -> Optional[Dict[str, Tuple[int, int, int]]]:
    """Меню за избор на цветове за играчите."""
    font = pygame.font.SysFont("comicsans", 40)
...
def board_size_menu() -> Optional[Tuple[int, int]]:
    """Меню за избор на размер на дъската (редове и колони)."""
    font = pygame.font.SysFont("comicsans", 40)
    clock = pygame.time.Clock()
...
def main() -> None:
    """Основната функция за инициализиране на играта."""
    mode = menu()
